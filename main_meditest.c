...
struct point init_point(int id_num, double att[], int lable)
{
    ...
    temp.lable = lable;
    if(temp.lable == 0){
        printf("DID NOT RECIEVE VALID VALUE IN INIT_POINT\n");
        exit(-1);
    }
    return temp;
}
...
bool shouldHalt(struct point old_centroid[], struct point centroid[], int k)
{
    for (int i = 0; i < k; i++)
    {
        //printf("GOING TO HALT: %d\n", !compare_points(old_centroid[i], centroid[i]));
...
{
    double minimum = DBL_MAX, candidate = 0;
    int target;
    Cluster clust[k];
    //This for loop finds the minimum.
    for (int i = 0; i < k; i++)
    {
        candidate = distanceCalculator(&p, &cent[i]);
...
struct cluster *kmeans(struct point *D, int n, int k)
{
    ...
    for(int i = 0; i < n; i++){
        if((D+i)->lable == 0){
            //print_point((D+i));
            printf("BAD DATA POINT WAS PASSED\n");
            exit(-1);
        }
    }
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        do {
            while((D+pick)->lable != lab){ pick = (rand() % n); } 
        } while (!is_unique(last_picked, pick, n) && (D+pick)->lable != lab);
        ...
        last_picked[i] = pick;
        *(C + i) = init_cluster((D + pick), 1); //The first point of each cluster is pointing at the address of memory for centroid array.
    }
    ...
    while (!shouldHalt(old_centroids, centroid, k) || amount != n)
    {
        ...
        if (w < n)
        {
            //printf("THIS IS THE POINT GOING IN\n");
            //print_point(&D[w]);
            ...
            //!shouldHalt(centroid, old_centroids, k);
            E = eCalculator(C, centroid, k);
            for(int j = 0; j < k; j++){
                for(int i = 0; i < (C+j)->n; i++){
                    if((C+j)->c[i].lable != 2 && (C+j)->c[i].lable != 4){
                        printf("RECIEVED INVALID VALUE 1 at: %d\n", w);
                        print_point(&(C+j)->c[i]);
                        exit(-1);
                    }
        ...
        {   //If we have assigned the points we need to adjust the clusters.
            ...
            if (fabs(E - E_previous) > 0.001)//  E < E_previous
...
double Accuracy(Cluster *C)
{
    ...
    // 4 malignant in cluster 0
    // 2 benign in cluster 1
    // 458 benign (65.5%)
    // 241 Malignant (34.5%)
    double TP, FP, TN, FN;
    TP = 0;
    ...
    printf("number in first cluster: %d\n", C->n);
    for (int i = 0; i < C->n; i++)
    {
            
        if(C->c[i].lable != 2 && C->c[i].lable != 4) {
            printf("RECIEVED INVALID VALUE 2\n");
            print_point(&C->c[i]);
            exit(1); 
        }
        switch (C->c[i].lable)
        {
            case 2: FP = FP + 1; break;
            
            case 4: TP = TP + 1; break;
        }
    }
    printf("number in second cluster: %d\n", (C+1)->n);
    for (int i = 0; i < (C+1)->n; i++)
    {
        
        if((C+1)->c[i].lable != 2 && (C+1)->c[i].lable != 4) {
            printf("RECIEVED INVALID VALUE 3\n");
            print_point(&(C+1)->c[i]);
            exit(1);
        }
        switch ((C+1)->c[i].lable)
        {
            case 2: TN = TN + 1; break;
            
            case 4: FN = FN + 1; break;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < valid; i++){
        if((p+i)->lable == 0){
            printf("DID NOT READ ALL VALUES CORRECTLY!\n");
            exit(-1);
        }
    }
    ...
    for (int i = 0; i < valid; i++)
    {
        ...
        //printf("POINTS GETTING INITIALIZED\n");
        *(D + i) = init_point((p + count)->id_num, (p + count)->att, (p + count)->lable);
        //print_point(D+i);
        count++;

        if((D+i)->lable == 0){
            printf("RECIEVED INVALID VALUE 0\n");
            print_point((D+i));
            exit(-1);
        }
    }
